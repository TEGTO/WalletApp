...
namespace WalletApi.Features.TransactionsListFeature.Command.GetBalance.Tests
{
    [TestFixture]
    internal class GetBalanceQueryHandlerTests
    {
        ...
        public async Task Handle_ValidRequest_ReturnsBalanceResponse()
        {
            ...
            Assert.That(result.CardLimit - result.CardBalance, Is.EqualTo(result.AvailableBalance));
        }
        [Test]
        ...
        public async Task Handle_MultipleRequests_ReturnsRandomBalances()
        {
            ...
            // Assert
            Assert.That(results.Select(r => r.CardBalance).Distinct().Count(), Is.GreaterThan(1), "Random balance generation failed to produce distinct values.");
        }
    }
}